...
function ImageUploader({ onImageSelect }) {
  ...
  const sendImageToBackend = async (file) => {
    ...
    try {
      const response = await axios.post('http://tu-direccion-backend/api/upload', formData);
      console.log(response.data); // respuesta del backend
      

    } catch (error) {
      console.error('Hubo un error al subir la imagen:', error.response ? error.response.data : error.message);
    } finally {
      setUploading(false);
    }
  };
